...
#include "graphics/texture/dielectric.hpp"
...
{
    constexpr size_t nx = 1600;
    constexpr size_t ny = 800;
    ...
    scene->add(std::make_shared<sphere<double, Color>>(Point(0, 0, -1), 0.5, std::make_shared<Lambertian>(Color{ 0.1, 0.2, 0.5 })));
    scene->add(std::make_shared<sphere<double, Color>>(Point(0, -100.5, -1), 100, std::make_shared<Lambertian>(Color{ 0.8,0.8,0.0 })));
    scene->add(std::make_shared<sphere<double, Color>>(Point(1, 0, -1), 0.5, std::make_shared<Metal>(Color{ 0.8,0.6,0.2 }, 0.2)));
    scene->add(std::make_shared<sphere<double, Color>>(Point(-1, 0, -1), 0.5, std::make_shared<Glass>(1.5)));
    ...
    save_image("rtiow_07_glass.png", img);
