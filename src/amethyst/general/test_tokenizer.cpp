...
int main(int argc, const char** argv)
{
	...
	try
	{
		...
		TEST_RESULT_NAMED(
			...
			tokenize("A B C D E F G H I J", NULL) == std::vector<string>(1, "A B C D E F G H I J"),
			passed,
			ERROR_TEXT(failed));

		const string junk1 = "A B	C D\nE F G";
		const string foo1[] = { "A", "B", "C", "D", "E", "F", "G" };
		std::vector<string> foo1_vec(foo1, foo1 + sizeof(foo1) / sizeof(*foo1));
		std::vector<string> result1_vec = tokenize(junk1);
		TEST_RESULT_NAMED(
