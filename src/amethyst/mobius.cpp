...
template <class T>
std::shared_ptr<image<T>> mobius_transform(
    const std::shared_ptr<image<T>>& input, const complex<T>& a, const complex<T>& b,
    ...
    )
{
    ...
    std::shared_ptr<image<T>> output(new image<T>(nx, ny));
...
int main(int argc, const char** argv)
{
    ...
    std::shared_ptr<image<float_type>> input = tga.input(input_file);
    std::shared_ptr<image<float_type>> output = mobius_transform(
