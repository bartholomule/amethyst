...
namespace amethyst
{
    ...
    public:
        ...
        const std::vector<const shape<T>*>& get_containers() const { return *all_containers; }
        std::vector<intersection_info<T>> get_all_intersections() const { return *all_intersections; }

        void set_shape(const shape<T>* s) { shape_hit = conditional_shape(s, true); }
        ...
        void set_containers(const std::vector<const shape<T>*>& cv) { all_containers = conditional_shape_vector(cv, true); }
        void set_all_intersections(const std::vector<intersection_info<T>>& intersections) { all_intersections = conditional_intersection_list(intersections, true); }
