...
int main(int argc, const char** argv)
{
	...
	if( parse_file_data(filename, data) )
	{
		...
		raster<quick_vector<coord2<number_type> > > samples = get_samples(data.width, data.height,
			...
			rc_pointer<generator>(new regular_sample_2d<number_type>()));
		...
		output.output(string_format("cs5600_program04_antialiased-%1spp-%2.tga", samples(0,0).size(), data.gamma), image, data.gamma);
		output.output(string_format("cs5600_program04_antialiased-%1spp-%2.tga", samples(0,0).size(), 1.7), image, 1.7);
		...
		rasterize_triangles(data, image, samples);
		std::cout << "Writing data to file.." << std::endl;
		output.output(string_format("cs5600_program04-%1spp-%2.tga", samples(0,0).size(), data.gamma), image, data.gamma);
		output.output(string_format("cs5600_program04-%1spp-%2.tga", samples(0,0).size(), 1.7), image, 1.7);
	}
	else
	{
