...
#include <graph_math/point3.h>
#include <graph_math/vector3.h>
#include <cassert>
...
template <class T>
matrix_4x4<T>::matrix_4x4<T>(const T dat[16])
{
  for(int i = 0; i < 16; ++i)
  {
    data[i] = dat[i];
...
template <class T>
matrix_4x4<T>::matrix_4x4<T>(const T dat[16], const T inv[16])
{
  for(int i = 0; i < 16; ++i)
  {
    data[i] = dat[i];
...
void matrix_4x4<T>::set_rotate_x(T theta)
{
  ...
  for(int row = 0; row < 4; ++row)
    for(int col = 0; col < 4; ++col)
      invese_data[LINEAR(row,col)] = data[LINEAR(col,row)];
}
template <class T>
