	...
	rc_pointer<generator> sampler)
{
	...
	for( int y = 0; y < height; ++y )
	{
		...
		{
			quick_vector<coord2<number_type> > samples =
...
int main(int argc, const char** argv)
{
	...
	typedef std::pair<const char*const, rc_pointer<generator> > gen_pair;
	quick_vector<gen_pair> generators;

	generators.push_back(gen_pair("regular", rc_pointer<generator>(new regular_sample_2d<number_type>())));
	generators.push_back(gen_pair("random", rc_pointer<generator>(new random_sample_2d<number_type>())));
	generators.push_back(gen_pair("jitter", rc_pointer<generator>(new jitter_sample_2d<number_type>())));
	generators.push_back(gen_pair("multi_jitter", rc_pointer<generator>(new multi_jitter_sample_2d<number_type>())));
	generators.push_back(gen_pair("poisson", rc_pointer<generator>(new poisson_sample_2d<number_type>(default_random<number_type>(), 1.0/64))));
	generators.push_back(gen_pair("nrooks", rc_pointer<generator>(new nrooks_sample_2d<number_type>())));
