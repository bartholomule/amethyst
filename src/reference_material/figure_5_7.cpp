...
int main(int argc, char** argv)
{
	...
	rc_pointer<shape<float_type> > sph_ptr1(new sphere<float_type>(point(0,0,0), 10));
	sl.add(sph_ptr1);
   ...
   float_type ambience(0.05);
	...
	for( int y = 0; y < HEIGHT; ++y )
	{
		for( int x = 0; x < WIDTH; ++x )
		{
			...
			quick_vector<coord2<float_type> > samples = gen_2d->get_samples(SAMPLES_PER_PIXEL);
	...
	output.output("figure_5_7.tga", image);
	return 0;
}
