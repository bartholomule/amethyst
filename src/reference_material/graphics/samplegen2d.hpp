...
namespace amethyst
{
	...
	public:
		...
		virtual quick_vector<coord2<T> > get_samples(int num_samples);
	...
	*/
	...
	{
		quick_vector<coord2<T> > v = get_samples(num_samples);
		for( typename quick_vector<coord2<T> >::const_iterator iter = v.begin();
			  iter != v.end();
			  ++iter )
		{
			o(*iter);
	...
	template <class T>
	quick_vector<coord2<T> > random_sample_2d<T>::get_samples(int num_samples)
	{
	...
	template <class T>
	quick_vector<coord2<T> > nrooks_sample_2d<T>::get_samples(int num_samples)
	{
		quick_vector<coord2<T> > vec(num_samples);
	...
	{
		quick_vector<coord2<T> > vec = get_samples(num_samples);
		for( typename quick_vector<coord2<T> >::const_iterator iter = vec.begin();
			  iter != vec.end();
			  ++iter )
		{
			pf(*iter);
	...
	template <class T>
	quick_vector<coord2<T> > multi_jitter_sample_2d<T>::get_samples(int num_samples)
	{
	...
	template <class T>
	quick_vector<coord2<T> > poisson_sample_2d<T>::get_samples(int num_samples)
	{
		quick_vector<coord2<T> > samples(num_samples);
	...
	{
		quick_vector<coord2<T> > v = get_samples(num_samples);
		for(typename quick_vector<coord2<T> >::iterator vp = v.begin();
			vp != v.end();
			++vp)
		{
			pf(*vp);
