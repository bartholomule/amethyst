/*
 * $Id: dynamic_sphere.hpp,v 1.1 2004/06/01 04:01:37 kpharris Exp $
 *
...
namespace amethyst
{
					  ...
					  const intersection_requirements& requirements) const
  {
	...
	{
	  intersection.set_normal(unit(intersection.get_first_point() - my_center));
	}
  ...
  bool dynamic_sphere<T>::intersects_ray(const ray_parameters<T>& ray,
					 ...
					 const intersection_requirements& requirements) const
  {
    T my_radius = get_radius(ray.get_time());
    point3<T> my_center = get_center(ray.get_time());

    //    std::cout << "Center(" << ray.get_time() << ")=" << my_center << std::endl;
    //    std::cout << "Radius(" << ray.get_time() << ")=" << my_radius << std::endl;    
  ...
  template <class T>
  std::string dynamic_sphere<T>::to_string(const std::string& indent,
                                   const std::string& level_indent) const
  {
    return ( indent + "dynamic_sphere\n" +
             indent + "{\n" +
             dynamic_sphere<T>::internal_members(indent + level_indent, false) +
             indent + "}" );
  }  
