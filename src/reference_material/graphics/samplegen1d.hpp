...
namespace amethyst
{
  ...
  public:
    ...
    virtual quick_vector<T> get_samples(int num_samples);
  ...
  {
    const quick_vector<T> v = get_samples(num_samples);
    for( typename quick_vector<T>::const_iterator iter = v.begin();
	 iter != v.end();
	 ++iter )
    {
      o(*iter);
  ...
  template <class T>
  quick_vector<T> random_sample_1d<T>::get_samples(int num_samples)
  {
    quick_vector<T> v;
    int i;
  ...
  template <class T>
  quick_vector<T> jitter_sample_1d<T>::get_samples(int num_samples)
  {
    int x;
    quick_vector<T> v;
  ...
  template <class T>
  quick_vector<T> poisson_sample_1d<T>::get_samples(int num_samples)
  {
    quick_vector<T> samples(num_samples);
  ...
  {
    quick_vector<T> v = get_samples(num_samples);
    for(typename quick_vector<T>::iterator vp = v.begin();
        vp != v.end();
        ++vp)
    {
      pf(*vp);
