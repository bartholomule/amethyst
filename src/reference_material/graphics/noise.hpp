...
namespace amethyst
{
	...
	{
	public:
		noise(rc_pointer<Random<T> > rnd = new default_random<T>(),
        int array_size = 256);
	...
	protected:
		...
		rc_pointer<Random<T> > rnd_gen;

	private:
		void create_arrays();

		int size_of_arrays;
		quick_vector<int> P_array;
		quick_vector<coord3<T> > G_array;
	};

	template <class T>
	noise<T>::noise(rc_pointer<Random<T> > rnd, int array_size):
		rnd_gen(rnd),
