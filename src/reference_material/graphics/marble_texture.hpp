...
namespace amethyst
{
  ...
  public:
    ...
    marble_texture(T line_width = 0.5, T scale = T(1), int octaves = 8, const rc_pointer<Random<T> >& rnd = rc_pointer<Random<T> >(new default_random<T>()));
    marble_texture(const color_type& c0, const color_type& c1, const color_type& c2, T line_width = 0.5, T scale = T(1), int octaves = 8, const rc_pointer<Random<T> >& rnd = rc_pointer<Random<T> >(new default_random<T>()));
    marble_texture(const interp_type& colors, T line_width = 0.5, T scale = T(1), int octaves = 8, const rc_pointer<Random<T> >& rnd = rc_pointer<Random<T> >(new default_random<T>()));
    ...
    virtual std::string name() const { return "marble_texture"; }
  private:
    T m_freq;
    T m_scale;
    noise<T> m_noise;
  ...
  template<typename T, typename color_type>
  marble_texture<T,color_type>::marble_texture(T line_width, T scale, int octaves, const rc_pointer<Random<T> >& rnd)
  ...
  template<typename T, typename color_type>
  marble_texture<T,color_type>::marble_texture(const color_type& c0, const color_type& c1, const color_type& c2, T line_width, T scale, int octaves, const rc_pointer<Random<T> >& rnd)
    : solid_texture<T,color_type>()
    , m_freq(M_PI / line_width)
    , m_scale(scale)
    , m_noise(rnd)
  ...
  template<typename T, typename color_type>
  marble_texture<T,color_type>::marble_texture(const typename marble_texture<T,color_type>::interp_type& colors, T line_width, T scale, int octaves, const rc_pointer<Random<T> >& rnd)
    : solid_texture<T,color_type>()
    , m_freq(M_PI / line_width)
    , m_scale(scale)
    , m_noise(rnd)
